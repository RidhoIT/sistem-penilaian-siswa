...
// //   const [showPelanggaranWarning, setShowPelanggaranWarning] = useState(false);
...
// //       if (document.hidden) {
// //         reportPelanggaran("TAB_SWITCH", "Siswa berpindah tab");
// //       }
// //     };
...
// //       );
// //       const data = await res.json();
// //       setPelanggaranCount(data.jumlahPelanggaran || 0);
// //       setShowPelanggaranWarning(true);
// //       setTimeout(() => setShowPelanggaranWarning(false), 4000);
// //       if (data.dihentikan) {
// //         await submitUjian("DIHENTIKAN");
// //       }
// //     } catch (_) {}
// //   }
...
// //   async function simpanJawaban(soalId: string, jawabanDipilih: string, isRagu: boolean) {
// //     if (!sesiId) return;
// //     try {
// //       await fetch(
...
// //           method: "POST",
// //           headers: { "Content-Type": "application/json" },
// //           body: JSON.stringify({ soalId, jawaban: jawabanDipilih, isRagu }),
// //         }
// //       );
// //     } catch (_) {}
// //   }
...
// //   async function submitUjian(alasan: string = "SELESAI") {
...
// //       const data = await res.json();
// //       router.push(
// //         `/siswa/selesai-ujian/${ujianId}?nilai=${data.nilaiAkhir}&benar=${data.nilaiBenar}&salah=${data.nilaiSalah}&lulus=${data.lulus}`
// //       );
// //     } catch (_) {
// //       setSubmitting(false);
// //     }
...
// //   const isWarning = sisaWaktu <= 300 && sisaWaktu > 0; // 5 menit terakhir
...
// //           <h2 className="text-base font-bold text-zinc-900 mb-1">Gagal Memuat Ujian</h2>
...
// //   return (
// //     <div className="min-h-screen bg-[#f8f9fb] select-none">
// //       {/* ── Pelanggaran Warning ── */}
// //       {showPelanggaranWarning && (
// //         <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 bg-red-600 text-white px-5 py-3 rounded-xl shadow-lg flex items-center gap-2.5 text-sm font-semibold animate-bounce">
// //           <AlertTriangle className="w-4 h-4" />
// //           Pelanggaran terdeteksi! ({pelanggaranCount}/3)
// //         </div>
// //       )}
...
// //             <span className="text-[13px] font-semibold text-zinc-700">Ujian Berlangsung</span>
// //           </div>

// //           <div className="flex items-center gap-2">
// //             <span className="text-[12px] text-slate-500">
// //               {dijawab}/{soalCount} dijawab
// //             </span>
// //             <div
...
// //       <div className="max-w-5xl mx-auto px-4 py-5 grid grid-cols-1 md:grid-cols-[240px_1fr] gap-4">
// //         {/* ── Sidebar Navigasi ── */}
...
// //               <p className="text-[11px] font-bold text-zinc-700">{Math.round((dijawab / soalCount) * 100)}%</p>
// //             </div>
// //             <div className="w-full bg-slate-100 rounded-full h-1.5 mb-3">
// //               <div
// //                 className="bg-zinc-900 h-1.5 rounded-full transition-all"
// //                 style={{ width: `${(dijawab / soalCount) * 100}%` }}
// //               />
// //             </div>
// //             <div className="flex items-center gap-3 text-[11px]">
// //               <span className="flex items-center gap-1 text-emerald-600 font-semibold">
// //                 <span className="w-2 h-2 bg-emerald-500 rounded-sm inline-block" /> {dijawab} dijawab
// //               </span>
// //               <span className="flex items-center gap-1 text-slate-400 font-semibold">
// //                 <span className="w-2 h-2 bg-slate-200 rounded-sm inline-block" /> {belumDijawab} belum
// //               </span>
// //             </div>
// //           </div>
...
// //           {/* Navigator grid */}
// //           <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
// //             <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-3">Navigasi Soal</p>
...
// //               </div>
// //               <div className="flex items-center gap-2">
// //                 <span className="w-4 h-4 bg-amber-100 border border-amber-200 rounded" />
...
// //             disabled={submitting}
// //             className="w-full flex items-center justify-center gap-2 py-3 px-4 bg-zinc-900 text-white rounded-xl text-[13px] font-bold hover:bg-zinc-800 disabled:opacity-60 transition-all"
// //           >
...
// //                     </span>
// //                     {currentSoal.topik && (
// //                       <span className="text-[10px] text-slate-400">{currentSoal.topik}</span>
// //                     )}
// //                   </div>
// //                 </div>
// //                 <div className="flex items-center gap-2">
...
// //                     {ragu[currentSoal.id] ? "Ragu-ragu" : "Tandai Ragu"}
// //                   </button>
...
// //                 <p className="text-[15px] text-zinc-900 leading-relaxed mb-5">
...
// //                     alt="Gambar soal"
// //                     className="max-w-sm rounded-xl mb-5 border border-slate-200"
// //                   />
// //                 )}
...
// //                       .filter((o) => o.val)
// //                       .map((opt) => {
// //                         const isSelected = jawaban[currentSoal.id] === opt.val;
// //                         return (
// //                           <button
// //                             key={opt.key}
// //                             onClick={() => handlePilihJawaban(currentSoal.id, opt.val!)}
...
// //                             </div>
// //                             <span className={`text-[13px] flex-1 ${isSelected ? "font-semibold text-zinc-900" : "text-zinc-700"}`}>
// //                               {opt.val}
// //                             </span>
// //                           </button>
// //                         );
...
// //                     className="w-full px-4 py-3 border border-slate-200 rounded-xl text-[13px] text-zinc-900 focus:outline-none focus:ring-2 focus:ring-zinc-900/20 resize-none"
// //                   />
// //                 )}
// //               </div>
...
// //                     className="flex items-center gap-1.5 px-4 py-2 bg-zinc-900 text-white rounded-lg text-[12px] font-semibold hover:bg-zinc-800"
// //                   >
...
// //                   <button
// //                     onClick={() => setShowConfirm(true)}
// //                     className="flex items-center gap-1.5 px-4 py-2 bg-emerald-600 text-white rounded-lg text-[12px] font-semibold hover:bg-emerald-700"
// //                   >
// //                     <Send className="w-3.5 h-3.5" /> Selesai
// //                   </button>
// //                 )}
// //               </div>
// //             </div>
...
// //             </div>
// //             <h3 className="text-base font-bold text-zinc-900 text-center mb-1">Kumpulkan Ujian?</h3>
// //             <p className="text-[13px] text-slate-500 text-center mb-4">
// //               Anda telah menjawab <strong className="text-zinc-900">{dijawab}</strong> dari{" "}
// //               <strong className="text-zinc-900">{soalCount}</strong> soal.
// //               {belumDijawab > 0 && (
// //                 <span className="text-amber-600 font-semibold"> {belumDijawab} soal belum dijawab.</span>
// //               )}
// //             </p>
// //             {belumDijawab > 0 && (
// //               <div className="flex items-center gap-2 p-3 bg-amber-50 border border-amber-100 rounded-xl mb-4">
// //                 <AlertTriangle className="w-4 h-4 text-amber-600 flex-shrink-0" />
// //                 <p className="text-[11px] text-amber-700">Soal yang belum dijawab tidak akan mendapat nilai.</p>
// //               </div>
// //             )}
// //             <div className="flex gap-3">
// //               <button
// //                 onClick={() => setShowConfirm(false)}
// //                 className="flex-1 py-2.5 border border-slate-200 rounded-xl text-[13px] font-semibold hover:bg-slate-50"
// //               >
// //                 Kembali
// //               </button>
// //               <button
// //                 onClick={() => { setShowConfirm(false); submitUjian("SELESAI"); }}
// //                 disabled={submitting}
// //                 className="flex-1 py-2.5 bg-zinc-900 text-white rounded-xl text-[13px] font-bold hover:bg-zinc-800 disabled:opacity-60 flex items-center justify-center gap-2"
// //               >
...
//       const data = await res.json();
//       router.push(
//         `/siswa/selesai-ujian/${ujianId}?nilai=${data.nilaiAkhir}&benar=${data.nilaiBenar}&salah=${data.nilaiSalah}&lulus=${data.lulus}&durasi=${data.durasi || 0}&totalSoal=${soalList.length}&pelanggaran=${pelanggaranCount}`
//       );
//     } catch (_) {
//       setSubmitting(false);
//     }
...
//       {/* ── SIDEBAR ── */}
//       <aside
//         className={`fixed left-0 top-0 w-[280px] h-full z-50 bg-white border-r border-slate-200 flex flex-col transition-transform duration-300 md:translate-x-0 ${
...
//                   }}
//                   className={`w-full aspect-square rounded-lg text-xs font-bold flex items-center justify-center border transition-all relative ${
//                     isActive
...
//             {/* Timer (mobile) */}
//             <div
//               className={`md:hidden flex items-center gap-1.5 px-3 py-1.5 rounded-lg font-mono font-bold text-sm ${
//                 isDanger
//                   ? "bg-red-100 text-red-700 animate-pulse"
//                   : isWarning
//                   ? "bg-amber-100 text-amber-700"
...
//                             />
//                             <div
//                               className={`flex items-center gap-4 px-5 h-14 border rounded-xl transition-all ${
//                                 isSelected
